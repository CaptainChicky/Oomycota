...
The queue is a slide-up panel accessed from the queue button on the player bar. It shows the current track with playback controls (play/pause, skip, progress bar) at the top, then "Up Next" with the real play order and per-track remove buttons, then "History" of already-played tracks greyed out. There's a "Clear" button to empty the queue. Right click (desktop) or long-press (mobile) any track anywhere for a context menu with "Play Next" (inserts at front of Up Next) and "Add to Queue" (appends to end). **IF YOU DO THIS ON ACCIDENT, JUST CLICK THE TOP OF THE PAGE (the part that has no response to clicking, next to the oomycota title on the left) AND IT WILL CLOSE THE CONTEXT MENU.**
